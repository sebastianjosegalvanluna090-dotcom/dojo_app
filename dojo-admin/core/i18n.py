...
def trf(key: str, fallback: str, **kwargs) -> str:
    """Like tr() but with a fallback string when the key is missing from the locale file.
    
    Use this for any user-visible text that must NEVER appear as a raw key on screen.
    """
    text = i18n._strings.get(key)
    if text is None:
        return fallback
    if kwargs:
        try:
